...
def run_bot_thread():
    """Telegram Bot ကို Thread အသစ်ဖြင့် သီးသန့် အလုပ်လုပ်စေခြင်း"""
    if not TELEGRAM_BOT_TOKEN:
        logger.warning("⚠️ TELEGRAM_BOT_TOKEN မရှိပါ။ Telegram Bot အလုပ်လုပ်မည်မဟုတ်ပါ။")
        return

    try:
        logger.info("🤖 Telegram Bot စတင်နေပါသည်...")
        tg_app = ApplicationBuilder().token(TELEGRAM_BOT_TOKEN).build()
        tg_app.add_handler(CommandHandler("start", start_command))
        tg_app.add_handler(CallbackQueryHandler(bot_button_handler))
        
        # stop_signals=None ဖြင့် FastAPI Signal များနှင့် ငြိစွန်းမှု မရှိစေရန် စီမံထားသည်
...
async def startup_event():
    ...
    # Bot ကို Thread ခွဲ၍ Run ခိုင်းခြင်း (Event Loop Conflict ဖြစ်မှုကို ဖြေရှင်းပေးပါသည်)
...
async def login_page():
    ...
    <head>
        ...
        <style>
            ...
            button { width: 100%; padding: 10px; background: #007bff; border: none; color: white; font-weight: bold; border-radius: 4px; cursor: cursor; }
        </style>
    </head>
    <body>
        <div class="login-box">
